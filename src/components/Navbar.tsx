...
export default function Navbar() {
  ...
  return (
    ...
    >
      <div
        className={`px-4 sm:px-6 py-2.5 sm:py-3 rounded-full flex items-center justify-between gap-3 sm:gap-6 md:gap-12 pointer-events-auto transition-all duration-500 max-w-xl w-full ${
          isScrolledPastHero
      ...
      >
        {/* Logo / Initials */}
        <a href="#" className="font-bold text-xs sm:text-sm tracking-[0.2em] sm:tracking-[0.25em] text-white hover:text-orange-400 transition-colors shrink-0">
        ...
        <nav className="flex items-center gap-3 sm:gap-6">
          {navItems.map((item) => (
            <a
              ...
              className="text-[11px] sm:text-xs font-mono tracking-wider text-white/70 hover:text-white transition-colors duration-200"
            >
              {item.label}
            </a>
          ))}
        ...
        <a
          ...
          className="text-[9px] sm:text-[10px] font-mono tracking-[0.1em] sm:tracking-[0.15em] uppercase px-3 sm:px-4 py-1.5 sm:py-2 rounded-full border border-orange-500/30 bg-orange-500/10 text-orange-300 hover:bg-orange-500 hover:text-black hover:border-orange-500 transition-all duration-300 flex items-center gap-1 sm:gap-1.5 shrink-0"
        >
          {/* Download icon */}
