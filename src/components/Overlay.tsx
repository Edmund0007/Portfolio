...
export default function Overlay({ scrollYProgress }: OverlayProps) {
  ...
  return (
    ...
    <div className="sticky top-0 h-screen w-full pointer-events-none z-10">
      ...
      <div
        className="absolute inset-0 flex flex-col justify-start pt-16 sm:justify-center sm:pt-0
                   items-start px-4 sm:px-6 md:px-16 lg:px-20 text-left"
      >
        <div className="max-w-[320px] sm:max-w-[600px] md:max-w-[760px] lg:max-w-[920px] p-3 sm:p-0 rounded-2xl bg-black/30 backdrop-blur-[3px] sm:bg-transparent sm:backdrop-blur-none border border-white/10 sm:border-none shadow-xl sm:shadow-none">
          <span className="font-mono text-[10px] sm:text-xs md:text-sm tracking-[0.2em] uppercase text-orange-400 font-bold mb-1 sm:mb-3 block">
            HELLO, IT&apos;S ME
          </span>
          <h1 className="text-2xl sm:text-4xl md:text-6xl lg:text-7xl font-black tracking-tight leading-[1.05] text-white drop-shadow-[0_4px_16px_rgba(0,0,0,0.9)]">
            EDMUND<br />AUGUSTINE
          </h1>

          {/* Animated Role Line: "I'm a [Role]" — Strictly Single Line Alignment */}
          <div className="mt-1 sm:mt-3 text-sm sm:text-xl md:text-3xl lg:text-4xl font-bold text-white flex items-center gap-1.5 sm:gap-3 min-h-[28px] sm:min-h-[48px] relative flex-wrap sm:flex-nowrap">
            ...
            <div className="relative overflow-visible flex items-center">
              <AnimatePresence mode="wait">
                <motion.span
                  key={roleIndex}
                  initial={{ x: 20, opacity: 0, scale: 0.94 }}
                  animate={{ x: 0, opacity: 1, scale: 1 }}
                  exit={{ x: -20, opacity: 0, scale: 0.94 }}
                  transition={{
                    type: "spring",
                    stiffness: 320,
                    damping: 22,
                  ...
                  className="bg-clip-text bg-gradient-to-r from-orange-400 via-amber-400 to-yellow-400 font-black inline-block leading-tight py-0.5 drop-shadow-[0_4px_20px_rgba(251,146,60,0.45)] drop-shadow-[0_2px_10px_rgba(0,0,0,0.95)] text-sm sm:text-xl md:text-3xl lg:text-4xl whitespace-normal sm:whitespace-nowrap"
                >
                  {roles[roleIndex]}
                </motion.span>
              </AnimatePresence>
          ...
          <p className="mt-1.5 sm:mt-4 text-[11px] sm:text-xs md:text-sm font-light text-white/80 tracking-wide leading-relaxed drop-shadow-[0_2px_8px_rgba(0,0,0,0.95)] max-w-[260px] sm:max-w-md">
          ...
          <motion.div style={{ opacity: scrollIndicatorOp }} className="mt-3 sm:mt-8 flex flex-col items-start gap-1 sm:gap-2">
            ...
            <div className="w-[14px] sm:w-[18px] h-[24px] sm:h-[30px] rounded-full border border-white/20 p-1 flex justify-center backdrop-blur-sm bg-black/20">
              <motion.div
                animate={{ y: [0, 6, 0] }}
      ...
      {/* ── SECTION 2 · Right side / Mobile Lower Area ─────────────────────── */}
      <motion.div
      ...
      >
        <div className="max-w-[290px] sm:max-w-[360px] lg:max-w-[410px] flex flex-col items-center sm:items-end text-center sm:text-right mb-2 sm:mb-0 p-3 sm:p-0 rounded-2xl bg-black/60 backdrop-blur-md sm:bg-transparent sm:backdrop-blur-none border border-white/10 sm:border-none shadow-2xl sm:shadow-none">
      ...
      {/* ── SECTION 3 · Right side / Mobile Lower Area ─────────────────────── */}
      <motion.div
